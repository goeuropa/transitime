...
public class AvlProcessor {
     ...
     *
     * @param vehicleState The vehicle to be made unpredictable
     * @param eventDescription A longer description of why vehicle being made unpredictable
     * @param vehicleEvent A short description from VehicleEvent class for labeling the event.
     */
            ...
            if (!traversedWaitStop) {
                ...
                AvlProcessor.getInstance()
                        .makeVehicleUnpredictable(
                                vehicleState.getVehicleId(), eventDescription, VehicleEvent.NO_PROGRESS);
     ...
     *
     * @param vehicleState For providing the temporal match and the AVL history. It is expected that
    ...
    private boolean handlePossibleVehicleDelay(VehicleState vehicleState) {
        ...
        // If no previous match then cannot determine if not making progress
        TemporalMatch previousMatch = vehicleState.getPreviousMatch(maxDelayedSecs * Time.MS_PER_SEC);
    ...
    public void matchNewFixForPredictableVehicle(VehicleState vehicleState) {
        ...
        logger.info("For vehicleId={} the best match is {}", vehicleState.getVehicleId(), bestTemporalMatch);

        // If didn't get a match then remember such in VehicleState
        if (bestTemporalMatch == null) vehicleState.incrementNumberOfBadMatches();
        ...
        // assignment.
        if (vehicleState.isPredictable() && vehicleState.lastMatchIsValid()) verifyRealTimeSchAdh(vehicleState);
     ...
     * @param vehicleState The VehicleState for the vehicle to be updated
    ...
    private void updateVehicleStateFromAssignment(
        ...
        vehicleState.setBlock(block, blockAssignmentMethod, assignmentId, predictable);
    ...
    private boolean matchVehicleToRouteAssignment(String routeId, VehicleState vehicleState) {
        ...
        logger.debug("Matching unassigned vehicle to routeId={}. {}", routeId, vehicleState);
        ...
        updateVehicleStateFromAssignment(
                bestMatch, vehicleState, BlockAssignmentMethod.AVL_FEED_ROUTE_ASSIGNMENT, routeId, "route");
    ...
    private boolean matchVehicleToBlockAssignment(Block block, VehicleState vehicleState) {
        ...
        logger.debug("Matching unassigned vehicle to block assignment {}. {}", block.getId(), vehicleState);
        ...
        if (bestMatch == null) {
            ...
            Trip trip = TemporalMatcher.getInstance().matchToLayoverStopEvenIfOffRoute(avlReport, potentialTrips);
            if (trip != null) {
    ...
    private boolean matchProblematicDueOtherVehicleHavingAssignment(TemporalMatch match, VehicleState vehicleState) {
        ...
        for (String vehicleId : vehiclesAssignedToBlock) {
            ...
            VehicleState otherVehicleState = VehicleStateManager.getInstance().getVehicleState(otherVehicleId);

            // If other vehicle that has assignment is schedule based then not
            // a problem to take its assignment away
            if (otherVehicleState.isForSchedBasedPreds()) return false;

            // If that other vehicle actually having any problem then not a
            // problem to take assignment away
            if (!otherVehicleState.isPredictable() || vehicleState.isDelayed()) return false;
        }
        ...
        for (String vehicleId : vehiclesAssignedToBlock) {
            ...
            if (block.shouldBeExclusive() || vehicleState.isForSchedBasedPreds()) {
                String description = "Assigning vehicleId="
                        ...
                        + ".";
                logger.info(description);
                makeVehicleUnpredictableAndGrabAssignment(vehicleState, description, VehicleEvent.ASSIGNMENT_GRABBED);
            }
        }
    }
    ...
    public boolean matchVehicleToAssignment(VehicleState vehicleState) {
        ...
        if (vehicleState.isPredictable() && vehicleState.hasNewAssignment(avlReport)) {
            ...
            makeVehicleUnpredictableAndTerminateAssignment(
                    vehicleState, eventDescription, VehicleEvent.ASSIGNMENT_CHANGED);
        }
    ...
    private void handlePredictableVehicleWithoutAvlAssignment(VehicleState vehicleState) {
        ...
        if (vehicleState.getBadAssignmentsInARow() < allowableBadAssignments.getValue()) {
            ...
            vehicleState.setBadAssignmentsInARow(vehicleState.getBadAssignmentsInARow() + 1);
        } else {
            ...
            makeVehicleUnpredictable(vehicleState.getVehicleId(), eventDescription, VehicleEvent.ASSIGNMENT_CHANGED);
        }
    }
    ...
    private boolean automaticalyMatchVehicleToAssignment(VehicleState vehicleState) {
        ...
        if (bestMatch != null) {
            ...
            logger.info("Auto matched vehicleId={} to a block assignment. {}", vehicleState.getVehicleId(), bestMatch);
    ...
    private boolean handlePossibleEndOfBlock(VehicleState vehicleState) {
        ...
        if (temporalMatch != null) {
            ...
            if (atStopInfo != null && atStopInfo.atEndOfBlock()) {
                ...
                makeVehicleUnpredictableAndTerminateAssignment(
                        vehicleState, eventDescription, VehicleEvent.END_OF_BLOCK);
    ...
    private void verifyRealTimeSchAdh(VehicleState vehicleState) {
        ...
        logger.debug("Confirming real-time schedule adherence for vehicleId={}", vehicleState.getVehicleId());

        // Determine the schedule adherence for the vehicle
        TemporalDifference scheduleAdherence = RealTimeSchedAdhProcessor.generate(vehicleState);
